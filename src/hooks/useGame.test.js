...
describe('initial state', () => {
  ...
  it('exposes maxGuesses of 7', () => {
    const { result } = renderHook(() => useGame())
    expect(result.current.maxGuesses).toBe(7)
  })
})
...
describe('submitGuess', () => {
  ...
  it('7th wrong guess sets status to lost', () => {
    const { result } = renderHook(() => useGame())
    act(() => {
      for (let i = 0; i < 7; i++) {
  ...
  it('7th wrong guess resets streak to 0', () => {
    const { result } = renderHook(() => useGame())
  ...
  it('hintsRevealed is capped at 6 regardless of additional wrong guesses', () => {
    const { result } = renderHook(() => useGame())
    act(() => {
      for (let i = 0; i < 10; i++) {
        result.current.submitGuess('Wrong Card ' + i)
    ...
    expect(result.current.gameState.hintsRevealed).toBe(6)
  })
})
...
describe('skipTurn', () => {
  ...
  it('7 skips result in lost status', () => {
    const { result } = renderHook(() => useGame())
    act(() => {
      for (let i = 0; i < 7; i++) {
